...
namespace quark {
    ...
    bool posix_tcp_socket::initialize_connection(const std::string & address,
        ...
        if(fd >= 0) {
            ...
            mReader = new quark::posix_tcp_socket_reader(this);
            mWriter = new quark::posix_tcp_socket_writer(this);
        } else {
            connected = false;
        }
    ...
    quark::reader * posix_tcp_socket::get_reader(void) const {
        quark::reader * reader;
    ...
    quark::writer * posix_tcp_socket::get_writer(void) const {
        quark::writer * writer;

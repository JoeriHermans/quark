...
#include <quark/network/proxy/socks.h>
...
namespace quark {
    ...
    bool socks_connect(const int fd,
        ...
        connected = quark::socks_connect(fd, address, port, "", "");
    ...
    bool socks_connect(const int fd,
        ...
        if(quark::socks_negotiate(fd, username, password)) {
            connected = quark::socks_request(fd, address, port);
        } else {
            connected = false;
        }
    ...
    bool socks_negotiate(const int fd,
        ...
        if(version == kSocksProtocolVersion) {
            ...
            case kSocksMethodUsernamePassword:
                connected = quark::socks_authenticate(fd, username, password);
                break;
            }
        }

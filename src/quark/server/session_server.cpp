...
#include <quark/server/session_server.h>
...
namespace quark {
    ...
    session_server::session_server(quark::server_socket * socket) :
        server(socket) {
        initialize();
    }
    ...
    void session_server::start(void) {
        ...
        mMainThread = new std::thread([this](){
                quark::server_socket * serverSocket;
                quark::session * session;
                quark::socket * socket;
    ...
    void session_server::update(void * argument) {
        ...
        session = static_cast<quark::session *>(argument);
        mMutexSessions.lock();

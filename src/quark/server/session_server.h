...
#include <quark/server/server.h>
#include <quark/server/session/session.h>
#include <quark/logger/logger.h>

// END Includes. /////////////////////////////////////////////////////

#ifndef QUARK_SESSION_SERVER_H_
#define QUARK_SESSION_SERVER_H_
...
class session_server : public server {
     ...
     */
    std::map<quark::session *, std::thread *> mSessions;
    ...
    virtual quark::session * allocate_session(quark::socket * socket) const = 0;
    ...
    session_server(quark::server_socket * socket);

...
#include <quark/threadpool/threadpool.h>
...
namespace quark {
    ...
    void threadpool::spawn_threads(const std::size_t numThreads) {
        ...
        for(std::size_t i = 0; i < numThreads; ++i) {
            thread = new std::thread(quark::threadpool::process, this);
    ...
    quark::threadpool_task * threadpool::next_task(void) {
        quark::threadpool_task * task;
    ...
    void threadpool::enqueue(quark::threadpool_task * task) {
        // Checking the precondition.
        assert(task != nullptr);
    ...
    void threadpool::process(quark::threadpool * pool) {
